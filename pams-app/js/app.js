...
const App = {
    ...
    async init() {
        ...
        try {
            ...
            this.applyDashboardLayoutFromUrl();
    ...
    /** Dashboard layout: default (full charts) | lean1 (split: pricing | win/loss) | lean2 (stacked, airy). */
    getDashboardLayout() {
        try {
            const fromUrl = new URLSearchParams(window.location.search).get('layout');
            if (fromUrl && ['default', 'lean1', 'lean2'].includes(fromUrl)) return fromUrl;
            const fromLs = localStorage.getItem('dashboardLayout');
            if (fromLs && ['default', 'lean1', 'lean2'].includes(fromLs)) return fromLs;
        } catch (_) { /* ignore */ }
        return 'default';
    },

    setDashboardLayout(layout) {
        const v = ['default', 'lean1', 'lean2'].includes(layout) ? layout : 'default';
        try { localStorage.setItem('dashboardLayout', v); } catch (_) { /* ignore */ }
        try {
        ...
        if (this.currentView === 'dashboard') {
            this.loadCardDashboard().catch(() => {});
        }
    },

    applyDashboardLayoutFromUrl() {
        try {
            const p = new URLSearchParams(window.location.search).get('layout');
            if (p && ['default', 'lean1', 'lean2'].includes(p)) {
                localStorage.setItem('dashboardLayout', p);
            }
        } catch (_) { /* ignore */ }
    },

    dashboardLayoutSwitcherHtml() {
        const cur = this.getDashboardLayout();
        const mk = (id, label) =>
            `<button type="button" class="dashboard-layout-btn ${cur === id ? 'active' : ''}" data-layout="${id}" onclick="App.setDashboardLayout('${id}'); return false;">${label}</button>`;
        return `
            <div class="dashboard-layout-switcher" role="group" aria-label="Dashboard layout">
    ...
    buildLeanDashboardHtml(opts) {
        ...
        const pricingEnabled = this.isFeatureEnabled('pricingCalculatorSync');
        const winLossEnabled = this.isFeatureEnabled('winLoss') && this.isDashboardVisible('winLoss');
        const pricingBlock = pricingEnabled ? `
            <div class="dashboard-lean-panel-inner">
                <div class="dashboard-lean-panel-head">
                    <h3>Synced pricing</h3>
                    <p class="text-muted small">Unlinked calculations from the pricing API. Log activity or delete after review.</p>
                </div>
                ${unlinkedPricing.length > 0 ? `
                <div class="table-responsive">
                    <table class="table table-sm dashboard-lean-table">
                ...
                ` : `<p class="text-muted small mb-0">No unlinked items. Use <strong>Sync all</strong> above.</p>`}
            </div>` : `<div class="dashboard-lean-panel-inner"><p class="text-muted small mb-0">Pricing sync is off for your role.</p></div>`;

        const winsRows = monthWins.slice(0, 12).map(w => `<tr><td>${escapeHtml(w.accountName)}</td><td>${escapeHtml(w.projectName)}</td><td>${escapeHtml(w.reason)}</td></tr>`).join('') ||
            '<tr><td colspan="3" class="text-muted">No wins this month</td></tr>';
        const lossRows = monthLosses.slice(0, 12).map(l => `<tr><td>${escapeHtml(l.accountName)}</td><td>${escapeHtml(l.projectName)}</td><td>${escapeHtml(l.reason)}</td></tr>`).join('') ||
            '<tr><td colspan="3" class="text-muted">No losses this month</td></tr>';
        const winLossBlock = winLossEnabled ? `
            <div class="dashboard-lean-panel-inner">
                <div class="dashboard-lean-panel-head">
                    <h3>Synced win / loss</h3>
                    <p class="text-muted small">Projects with win/loss in <strong>${escapeHtml(viewMonthName)}</strong></p>
                </div>
                <h4 class="dashboard-lean-subh">Wins (${monthWins.length})</h4>
                <div class="table-responsive">
                    <table class="table table-sm dashboard-lean-table">
                        <thead><tr><th>Account</th><th>Project</th><th>Notes</th></tr></thead>
                        <tbody>${winsRows}</tbody>
                    </table>
                </div>
                <h4 class="dashboard-lean-subh">Losses (${monthLosses.length})</h4>
                <div class="table-responsive">
                    <table class="table table-sm dashboard-lean-table">
                        <thead><tr><th>Account</th><th>Project</th><th>Notes</th></tr></thead>
            ...
            </div>` : `<div class="dashboard-lean-panel-inner"><p class="text-muted small mb-0">Win/Loss is hidden for your role.</p></div>`;

        const syncBar = (pricingEnabled || winLossEnabled) ? `
            <div class="dashboard-lean-sync-bar">
                <button type="button" class="btn btn-primary" id="dashboardUnifiedSyncBtn">Sync all</button>
                <span class="text-muted small">Updates pricing from API and refreshes accounts &amp; activities.</span>
            </div>` : '';

        const panelsClass = layoutMode === 'lean1' ? 'dashboard-lean-split' : 'dashboard-lean-stack dashboard-lean-v2';

        return `
            <div class="dashboard-lean ${layoutMode === 'lean2' ? 'dashboard-lean-v2-root' : ''}">
                <div class="dashboard-lean-toprow">
                    <div class="dashboard-lean-toprow-left">
                        ...
                        ${this.dashboardLayoutSwitcherHtml()}
                    </div>
                    ${isAdmin ? `
                    <div class="dashboard-lean-admin-month">
                        <label for="dashboardMonthSelectLean" class="text-muted small">Month</label>
                        <select id="dashboardMonthSelectLean" onchange="App.setDashboardMonthAndReload(this.value)" class="form-control form-control-sm">
                ...
                <div class="${panelsClass}">
                    <section class="dashboard-lean-panel">${pricingBlock}</section>
                    <section class="dashboard-lean-panel">${winLossBlock}</section>
                </div>
            </div>`;
    },

    bindLeanDashboardEvents(dashboardView, unlinkedPricing) {
        ...
        const unified = dashboardView.querySelector('#dashboardUnifiedSyncBtn');
        if (unified) {
            unified.addEventListener('click', async () => {
                unified.disabled = true;
                const prev = unified.textContent;
                unified.textContent = 'Syncing…';
                try {
                    ...
                    if (typeof DataManager !== 'undefined' && DataManager.invalidateCache) {
                        DataManager.invalidateCache('accounts', 'allActivities');
                    }
                    await this.loadCardDashboard();
                    if (typeof UI !== 'undefined' && UI.showNotification) {
                        UI.showNotification('Sync complete.', 'success');
                    }
                } catch (e) {
                    if (typeof UI !== 'undefined' && UI.showNotification) {
                        UI.showNotification((e && e.message) || 'Sync failed.', 'error');
                    }
                } finally {
                    unified.disabled = false;
                    unified.textContent = prev;
                }
            });
        }
    },
    ...
    async loadCardDashboard() {
        ...
        const layoutMode = this.getDashboardLayout();
        if (layoutMode === 'lean1' || layoutMode === 'lean2') {
            const leanHtml = this.buildLeanDashboardHtml({
                layoutMode,
                isAdmin,
                monthOptions,
                selectorValue,
            ...
            dashboardView.innerHTML = leanHtml;
            this.applyAppConfiguration();
            this.bindLeanDashboardEvents(dashboardView, unlinkedPricing);
            return;
        }
        ...
        if (activityModal && activityModal.offsetParent !== null) {
            ...
            if (hasData) {
                const formData = {
                    ...
                    rfxNotes: document.getElementById('rfxNotes')?.value || ''
                };
                Drafts.addDraft({
    ...
    loadDraftsView() {
        ...
        container.querySelectorAll('.draft-submit').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (p && p._activityUpdate === true && p.activity && typeof Activities !== 'undefined' && Activities.openActivityModal) {
                    if (typeof Drafts !== 'undefined' && Drafts.updateDraft) Drafts.updateDraft(draft.id, { errorMessage: '' });
                    const isInt = !!(p.activity.isInternal === true || draft.type === 'internal');
                    await Activities.openActivityModal({ mode: 'edit', activity: p.activity, isInternal: isInt, fromDraftId: draft.id });
                    App.loadDraftsView();
                    return;
                }
                if (p && p._pricingCalc === true) {
        ...
        container.querySelectorAll('.draft-edit').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (p && p._activityUpdate === true && p.activity && typeof Activities !== 'undefined' && Activities.openActivityModal) {
                    const isInt = !!(p.activity.isInternal === true || draft.type === 'internal');
                    Activities.openActivityModal({ mode: 'edit', activity: p.activity, isInternal: isInt, fromDraftId: draft.id });
                    App.loadDraftsView();
                    return;
                }
