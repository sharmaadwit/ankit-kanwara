...
const IMPORTED_ACTIVITIES_VERSION_KEY = 'importedDataVersion';
...
const DataManager = {
    ...
    async bootstrapFromMigrationSnapshot() {
        if (typeof window === 'undefined' || typeof fetch === 'undefined') {
            return;
        }

        const targetVersion =
            typeof window.__PAMS_IMPORTED_DATA_VERSION__ === 'string'
                ? window.__PAMS_IMPORTED_DATA_VERSION__
                : '';
        if (!targetVersion) {
            return;
        }

        const storedVersion = localStorage.getItem(IMPORTED_ACTIVITIES_VERSION_KEY) || '';
        const existingActivities = this.getActivities();
        const existingAccounts = this.getAccounts();

        if (existingActivities.length && existingAccounts.length && storedVersion === targetVersion) {
            return;
        }

        try {
            const response = await fetch('/data/importedData.snapshot.json', {
                cache: 'no-store'
            });
            if (!response.ok) {
                console.warn(
                    '[DataManager] Unable to fetch migration snapshot',
                    response.status,
                    response.statusText
                );
                return;
            ...
            if (!Array.isArray(records) || !records.length) {
                console.warn('[DataManager] Migration snapshot is empty.');
                return;
            }

            this._applyMigrationRecordsFromSnapshot(records, targetVersion);
        } catch (error) {
            console.error('[DataManager] Failed to load migration data', error);
        }
    },

    _applyMigrationRecordsFromSnapshot(records, version) {
        this.ensureDefaultUsers();
        const users = this.getUsers().map((user) => ({ ...user }));
        ...
        users.forEach((user) => {
            ...
            if (user.username) {
                userMap.set(user.username.trim().toLowerCase(), user);
            }
        });
        ...
        const resolveUser = (record) => {
            ...
            const baseName = record.assignedUserName || (email ? email.split('@')[0] : 'user');
        ...
        const ensureProject = (account, record) => {
            const name =
        ...
        records.forEach((record) => {
                ...
                internalActivities.push({
                    ...
                    timeSpent:
                        record.raw?.timeSpentType && record.raw?.timeSpentValue
                            ...
                            : null,
                    activityName:
                        record.projectName || record.raw?.internalActivityName || 'Internal Activity',
            ...
            const account = ensureAccount(record);
            const project = ensureProject(account, record);
        ...
        this.saveAccounts(accounts);
        this.saveActivities(activities);
        this.saveInternalActivities(internalActivities);

        if (version) {
            try {
                localStorage.setItem(IMPORTED_ACTIVITIES_VERSION_KEY, version);
            } catch (error) {
                console.warn('Unable to persist migration version flag', error);
            }
        }
        ...
        console.info('[DataManager] Imported migration dataset', {
